...
method_dic = {'mscluster':{'filename':'#Filename','scan':'#Scan','mass':'#ParentMass','rt_time':'#RetTime'},'falcon':{'filename':'filename','scan':'scan','mass':'precursor_mz','rt_time':'retention_time'}}
...
def mscluster_purity(cluster_results,matching_pairs_set):
    ...
    return cluster_results.groupby('#ClusterIdx').apply(lambda x: calculate_cluster_purity_weighted_avg(x, matching_pairs_set,'mscluster')), mscluster_results.groupby('#ClusterIdx').size()

def falcon_purity(cluster_results,matching_pairs_set):
    return cluster_results.groupby('cluster').apply(lambda x: calculate_cluster_purity_weighted_avg(x, matching_pairs_set,'falcon')), falcon_results.groupby('cluster').size()
...
if __name__ == "__main__":
    ...
    print('falcon size min:',falcon_size.min())
    print('mscluster size min:',mscluster_size.min())
