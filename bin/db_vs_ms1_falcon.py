...
if __name__ == "__main__":
    ...
    cluster_results = pd.read_csv('../data/cluster_info.tsv',sep='\t')  # Adjust file path and format accordingly
    ...
    cluster_purity = cluster_results.groupby('cluster').apply(lambda x: calculate_cluster_purity_avg(x, matching_pairs_set))
    ...
    merged_data = pd.merge(filtered_cluster_results, database_results,
                           left_on=['filename', 'scan', 'precursor_charge'],
                           right_on=['MzIDFileName', 'ScanNumber', 'Charge'],
                           how='inner')

    db_sizes = merged_data.groupby('cluster').size()
